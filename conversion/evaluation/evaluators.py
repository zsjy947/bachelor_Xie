...
def calc_info_extract_task_scores(list_structured_golden,
    ...
    for samp_golden, samp_predict in zip(list_structured_golden, list_structured_predict):
        ...
        if len(answer_predict)==0 and len(answer_golden)==0:
            f1_o.append(1)
        elif len(set_golden.intersection(set_predict)):
            tp1 = len(set_golden.intersection(set_predict))
            fp1 = len(set_predict.difference(set_golden))
            fn1 = len(set_golden.difference(set_predict))
            pre1 = tp1 / (tp1 + fp1)
            rec1 = tp1 /(tp1 +fn1)
    ...
    return precision, recall, f1,f1_o
