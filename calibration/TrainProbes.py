...
def define_model(input_dim,task_type:str):
    """
...
def evaluate_model(model, test_embeddings, test_labels,task_type):
    """
    ...
    if task_type == 'cla':
        ...
        loss = model.evaluate(test_embeddings, test_labels)
        return loss
...
def main():
    ...
    # Load the config
    try:
        config_parameters = load_config("calibration/config.json")
    except Exception as e:
    ...
    for idx in range(len(layers_to_process)):
        ...
        for count, (test_dataset, train_dataset, test_dataset_path) in enumerate(zip(test_datasets, train_datasets, dataset_paths)):
            ...
            train_embeddings = np.array([np.fromstring(correct_str(embedding), sep=',') for embedding in train_dataset[parameter].tolist()])
            train_labels = train_dataset['label']
            test_embeddings = np.array([np.fromstring(correct_str(embedding), sep=',') for embedding in test_dataset[parameter].tolist()])
            ...
            for i in range(repeat_each):
                ...
                # Define the model
                model = define_model(train_embeddings.shape[1],task_type)
                ...
                if task_type == 'cla':
                    ...
                    # Evaluate the model
                    loss,accuracy = evaluate_model(model, test_embeddings, test_labels,task_type)
                    ...
                    loss = evaluate_model(model, test_embeddings, test_labels,task_type)
            ...
            if save_probes:
                try:
                    ...
                    if should_remove_period:
                        model_path = os.path.join(probes_path, f"{model_name}_{abs(layers_to_process[idx])}_{dataset_names[count]}_recall_rp.h5")
                    else:
                            model_path = os.path.join(probes_path, f"{model_name}_{abs(layers_to_process[idx])}_{dataset_names[count]}_recall.h5")
            ...
            test_dataset_copy[f'average_probability'] = np.mean(all_probs_list, axis=0)
            ...
            new_path = original_path.with_name(original_path.name.replace('.csv', f'_{task_type}_predictions.csv'))

            # Save the modified copy as a new CSV file
            test_dataset_copy.to_csv(new_path, index=False,encoding='utf-8',columns=['statement','label','average_probability']) 
        # Print the results
        if task_type == 'cla' and not test_first_only:
